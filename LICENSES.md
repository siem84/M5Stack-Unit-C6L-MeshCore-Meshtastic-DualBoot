# Licensing and upstream projects

This project contains modifications associated with two independent upstream open-source projects.

## MeshCore

Upstream project:

https://github.com/meshcore-dev/MeshCore

MeshCore is distributed under the MIT License.

The MeshCore-side work includes the earlier M5Stack Unit C6L port developed by `siem84`, itself derived from MeshCore upstream, together with the later DualBoot modifications.

The MeshCore modification patch remains subject to the applicable upstream copyright and license terms.

## Meshtastic firmware

Upstream project:

https://github.com/meshtastic/firmware

Meshtastic firmware is distributed under the GNU General Public License v3.0 (GPL-3.0).

Modified Meshtastic source and firmware remain subject to the applicable GPL-3.0 requirements and upstream copyright notices.

## Independence

This C6L DualBoot project is an independent community modification.

It is not an official release of MeshCore, Meshtastic or M5Stack.

MeshCore, Meshtastic and M5Stack names and trademarks belong to their respective owners.
